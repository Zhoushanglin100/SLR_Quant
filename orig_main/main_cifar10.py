...
# import admm
# import admm1 as admm
# import admm.admm_itr_new as admm
import admm.admm_itr_cpy as admm
...
parser.add_argument('--update-lambda', type=int, default=1, metavar='N',
                    help='Choose whether to update initial rho in each iteration, 1-update, 0-not update')
parser.add_argument('--reg-lambda', type=float, default=1e-3, metavar='M',
...
def main():
    ...
    if args.logger:
        ...
        try:
            fldr_name = "logger/cifar10_"+args.arch+"_"+str(args.update_lambda)
            os.makedirs(fldr_name, exist_ok=True)
        except TypeError:
            raise Exception("Direction not create!")
        log_name = "cifar10_{}".format(args.arch) + "_{}".format(args.optimization) + "_{}".format(args.quant_type) + "_{}".format(args.num_bits) + "_{}".format(args.M) + "_{}".format(args.r) + "_{}".format(args.initial_s)
    ...
    # if args.arch == "resnet18":
    #     name_list = [# "layer1.0.conv1.weight","layer1.0.conv2.weight", "layer1.1.conv1.weight","layer1.1.conv2.weight",
    #                 "layer2.0.conv1.weight","layer2.0.conv2.weight", "layer2.1.conv1.weight","layer2.1.conv2.weight",
    #                 "layer3.0.conv1.weight","layer3.0.conv2.weight", "layer3.1.conv1.weight","layer3.1.conv2.weight",
    #                 "layer4.0.conv1.weight","layer4.0.conv2.weight", "layer4.1.conv1.weight","layer4.1.conv2.weight",
    #                 "fc.weight"]
    ...
    try:
        os.makedirs(args.save_dir+"/cifar10_"+args.arch+"_"+str(args.update_lambda), exist_ok=True)
    except TypeError:
        raise Exception("Direction not create!")
    model_path = os.path.join(args.save_dir, "cifar10_"+args.arch+"_"+str(args.update_lambda), args.save_model_name)
    ...
    if args.admm_quant:
        ...
        for epoch in range(1, args.epochs + 1):
            ...
            if best < acc:
                ...
                model_name = model_path + "_{}".format(args.arch) + "_{}".format(args.optimization) + "_{}".format(args.quant_type) + "_{}".format(args.num_bits) + "_{}".format(args.M) + "_{}".format(args.r) + "_{}".format(args.initial_s) + ".pt"
                model_new_name = model_path + "_{}".format(args.arch) + "_{}".format(args.optimization) + "_{}".format(args.quant_type) + "_{}".format(args.num_bits) + "_{}".format(args.M) + "_{}".format(args.r) + "_{}".format(args.initial_s) + ".pt"
...
def test(args, model, device, test_loader, criterion):
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            test_coss = criterion(output, target).item()
