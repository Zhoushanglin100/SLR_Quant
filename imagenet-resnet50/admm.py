...
class ADMM: #ADMM class (but also used for SLR training)
    def __init__(self, model, file_name, rho=0.1):
    ...
    if optimization == 'admm':
        ...
        if epoch != 1 and batch_idx == 0:
            ...
            for i, (name, W) in enumerate(model.named_parameters()): 
                ...
                ADMM.ADMM_Lambda[name] = ADMM.s*(W - ADMM.ADMM_Z[name]) + ADMM.ADMM_Lambda[name]  #Equation 5 #first update of Lambda
                ...
                ADMM.ADMM_Z[name] = W + ADMM.ADMM_U[name]  # Z(k+1) = W(k+1)+U[k] #Equation 6
            ...
            satisfied = Lagrangian(ADMM, model) #check if surrogate optimality condition is satisfied.

            ADMM.condition.append(satisfied)
...
def admm_masked_train(args, ADMM, model, device, train_loader, optimizer, epoch):
    ...
    if epoch == 1:
        # inialize Z variable
        # print("Start admm training quantized network, quantization type: {}".format(quant_type))
...
def combined_masked_retrain(args, ADMM, model, device, train_loader, optimizer, epoch):
    ...
    with open("./profile/" + config_file + ".yaml", "r") as stream:
        raw_dict = yaml.load(stream)
    ...
    for batch_idx, (data, target) in enumerate(train_loader):
        ...
        if batch_idx % log_interval == 0:
            ...
            print("({}) ({}) cross_entropy loss: {}".format(args.sparsity_type, optmzr, loss))
