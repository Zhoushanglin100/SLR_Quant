...
def z_u_update(args, model, device, epoch, iteration, batch_idx, name_list=None):
    ...
    if args.optimization == 'admm':
        if epoch != 1 and (epoch - 1) % args.admm_epochs == 0 and batch_idx == 0:
            ...
            for name, W in model.named_parameters():
                ...
                if args.update_lambda == 1:
                    print("++++++++++++ update rho")
                    model.rhos[name] = model.rhos[name] * 1.1
                elif args.update_lambda == 0:
    ...
    if args.optimization == 'savlr':
        ...
        if epoch != 1 and batch_idx == 0:
            ...
            for name, W in model.named_parameters():
                ...
                # (??) n1=torch.sqrt(torch.sum((model.W_k[name] - model.Z_k[name]) ** 2)).item()   #||W(k-1)-Z(k-1)||
                # (??) n2=torch.sqrt(torch.sum((W - model.Z_k[name]) ** 2)).item()                 #||W(k)-Z(k-1)||
            ...
            satisfied = Lagrangian1(model, name_list) 
            ...
            if satisfied == 1 or model.k==1: #if surr. opt. condition is satisfied or k==1
                ...
                for i, (name, W) in enumerate(model.named_parameters()):
                    ...
                    model.ADMM_Lambda[name] = model.rhos[name]*(W - model.Z_prev[name]) + model.ADMM_Lambda[name]  #Equation 4 #first update of Lambda
                    # model.ADMM_Lambda[name] = model.s*(W - model.Z_prev[name]) + model.ADMM_Lambda[name]  #Equation 4 #first update of Lambda

                    model.W_prev[name] = W               # why need? (???)
                    model.Z_prev[name] = model.Z[name]   # why need? (???)
    ...
    ###############################################################################################
            ...
            for i, (name, W) in enumerate(model.named_parameters()): 
                ...
                # (???) shouldn't keep W[k-1]? (not update)
            ...
            ###### check if surrogate optimality condition is satisfied #####
            satisfied = Lagrangian2(model, name_list)
            ...
            if satisfied == 1 or model.k==1: #if surr. opt. condition is satisfied or k==1
                ...
                for i, (name, W) in enumerate(model.named_parameters()):
                    ...
                    model.ADMM_Lambda[name] = model.rhos[name]*(W - model.Z[name]) + model.ADMM_Lambda[name]  # Lambda(k+1) = s*(W(k)-Z(k)) + Lambda(k+0.5) #EQUATION 9
                ...
                for i, (name, W) in enumerate(model.named_parameters()):
                    ...
                    model.ADMM_Lambda[name] = model.ADMM_Lambda[name]
                    model.U[name] = model.ADMM_Lambda[name]/model.rhos[name] 
...
def Lagrangian2(model, name_list):
    ...
    for i, (name, W) in enumerate(model.named_parameters()):
        ...
        # (???) admm_loss[name] = 0.5 * model.rhos[name] * (torch.norm(W - model.Z[name] + U, p=2) ** 2)
        ...
        # (???) admm_loss2[name] = 0.5 * model.rhos[name] * (torch.norm(W - model.Z_k[name] + U, p=2) ** 2)
    ...
    if Lag < Lag2+100000000: #if current Lag < previous Lag
        satisfied = 1
...
def Lagrangian1(model, name_list):
    ...
    for i, (name, W) in enumerate(model.named_parameters()):
        ...
        # (???) admm_loss[name] = 0.5 * model.rhos[name] * (torch.norm(W - model.Z_k[name] + U, p=2) ** 2)
        U_sum = U_sum + (0.5 * model.rhos[name] * (torch.norm(U, p=2) ** 2))

        admm_loss2[name] = 0.5 * model.rhos[name] * (torch.norm(model.W_prev[name] - model.Z_prev[name] + U, p=2) ** 2) #calculate prev Lagrangian
        # (???) admm_loss2[name] = 0.5 * model.rhos[name] * (torch.norm(model.W_k[name] - model.Z_k[name] + U, p=2) ** 2)
        U_sum2 = U_sum2 + (0.5 * model.rhos[name] * (torch.norm(U, p=2) ** 2))
    ...
    if Lag < Lag2-1000000000: #if current Lag < previous Lag
        satisfied = 1
