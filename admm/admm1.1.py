...
def project_to_centroid(args, model, W, name, device):
    ...
    num_iter_quant = 6  # default 20, 5 in paper
...
def z_u_update(args, model, device, epoch, batch_idx, name_list=None):
    ...
    if args.optimization == 'savlr':
        if epoch != 1 and batch_idx == 0:
            ...
            for name, W in model.named_parameters():
                ...
                n1=torch.sqrt(torch.sum((model.W_prev[name] - model.Z_prev[name]) ** 2)).item() #||W(k)-Z(k)||
                n2=torch.sqrt(torch.sum((W - model.Z_prev[name]) ** 2)).item()  #||W(k+1)-Z(k+1)||
                #n2=torch.sqrt(torch.sum((W - model.Z[name]) ** 2)).item()  #||W(k+1)-Z(k+1)||
            ...
            satisfied = Lagrangian1(model) #check if surrogate optimality condition is satisfied.
           
            model.condition1.append(satisfied)
               
            if satisfied == 1 or model.k==1: #if surr. opt. condition is satisfied or k==1
                model.k += 1 #increase k
                if total_n1 != 0 and total_n2 != 0:  #if norms are not 0, update stepsize
                    model.s = alpha * (model.s*total_n1/total_n2) 
                    print("savlr s:")
                    print(model.s)
                ...
                for i, (name, W) in enumerate(model.named_parameters()):
                    if ("weight" not in name) or (name not in name_list):
                        continue                
                    model.ADMM_Lambda[name] = model.s*(W - model.Z_prev[name]) + model.ADMM_Lambda[name]  #Equation 5 #first update of Lambda

                    model.ADMM_Lambda[name] = model.ADMM_Lambda[name] #keep the updated lambda
    ...
    ###############################################################################################
            ...
            for i, (name, W) in enumerate(model.named_parameters()): 
                ...
                model.Z[name] = W + model.U[name] 
                

                #_, updated_Z = weight_pruning( model.Z[name],
                #                              model.prune_ratios[name])  # equivalent to Euclidean Projection
                ...
                model.rhos[name] = model.rhos[name] * 1.1
                model.Z[name] = updated_Z #Z_(k+1)

            
                #n1=torch.sqrt(torch.sum((model.W_k[name] - model.Z_k[name]) ** 2)).item() #||W(k)-Z(k)||
                #n2=torch.sqrt(torch.sum((W - model.Z[name]) ** 2)).item()  #||W(k+1)-Z(k+1)||
                ...
                #model.conv_wz[name].extend([float(torch.sqrt(torch.sum((W-model.Z[name])**2)).item())])  
                #model.conv_zz[name].extend([float(torch.sqrt(torch.sum((model.Z[name]-model.Z_k[name])**2)).item())])


                #model.ADMM_Lambda[name] = model.s*(W - model.Z[name]) + model.ADMM_Lambda[name]  # Lambda(k+1) = s*(W(k+1) - Z(k+1)) +Lambda(k+0.5) #EQUATION 7
               

            satisfied = Lagrangian2(model) #check if surrogate optimality condition is satisfied.

            model.condition2.append(satisfied)
...
def Lagrangian1(model):
    ...
    for i, (name, W) in enumerate(model.named_parameters()):
        # print("+++++++++++++", name)
            ...
            # print("------------------")       
            # print("11111", name)
            # print(model.ADMM_Lambda[name])
            # # print(model.rhos[name])
            # print("------------------")
